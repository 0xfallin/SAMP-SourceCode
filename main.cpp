...
//#define UseSettings //Nyalain kalo mau buat Universial
const auto encryptedAddress = cryptor::create("139.162.24.177");
 unsigned short port = 7777; //ip dan port server kamu
...
void InitSAMP()
{
	Log("Lufzyy Samp loaded! Build time: " __DATE__ " " __TIME__);
...
void InitNetwork()
{
	if(!bNetworkInited && pSettings->Get().iFontOutline)
	{
		...
		#else
			pNetGame = new CNetGame(encryptedAddress.decrypt(), port, pSettings->Get().szNickName, "tesajacuy"); //pw server
		#endif
...
{	
	...
	{
		sprintf(buffer, "%sSamp_log.txt", g_pszStorage);
