...
uintptr_t CPad__ProcessPlayerWeapon_hook(uintptr_t thiz, uintptr_t ped)
{
	...
	{
		*pbyteCurrentPlayer = byteCurPlayer;
		result = CPad__ProcessPlayerWeapon(thiz, dwCurPlayerActor);
		*pbyteCurrentPlayer = 0;
	}
	else
	{
...
{
	//Log("CTaskSimpleUseGun__SetPedPosition");
	dwCurPlayerActor = (uintptr_t)ped;
	byteCurPlayer = FindPlayerNumFromPedPtr((uintptr_t)ped);
	byteInternalPlayer = *pbyteCurrentPlayer;
		...
		byteInternalPlayer == 0) // not local player and local player's keys set.
	{
		...
		*pbyteCurrentPlayer = byteCurPlayer;
		...
		// CTaskSimpleUseGun::SetPedPosition()
		(( uint32_t (*)(uintptr_t, uintptr_t *))(g_libGTASA+0x46D6AC+1))(thiz, ped);
...
void HookCPad()
{
	...
	InstallMethodHook(g_libGTASA+0x5C8610, (uintptr_t)CTaskSimpleUseGun__SetPedPosition_hook);				 // CPad::TaskUseGun
