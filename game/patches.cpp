...
void ApplyInGamePatches()
{
    ...
    *(uint8_t*)(g_libGTASA+0x4EE200) = 0x9B;
    ...
    // Display FPS
    *(bool *)(g_libGTASA + 0x008ED875) = true;
	
	//AUTOAIM
	RET(g_libGTASA+0x4A82D4);
	
	// CPlaceName::Process
    RET(g_libGTASA+0x45F1A4);
	
	// CHud::DrawBustedWastedMessage
	RET(g_libGTASA+0x3D62FC);
    ...
    //UPDATES
	NOP(g_libGTASA + 0x39844E, 2);  // CCarFXRenderer::Shutdown from CGame::Shutdown
    NOP(g_libGTASA + 0x39845E, 2);	// CCarFXRenderer::Shutdown from CGame::Shutdown
	RET(g_libGTASA + 0x541AA8); // CRealTimeShadowManager::ReturnRealTimeShadow from ~CPhysical
	RET(g_libGTASA + 0x541AC4); // CRealTimeShadowManager::Update
	RET(g_libGTASA + 0x543B04); // CShadows::RenderStaticShadows
	RET(g_libGTASA + 0x5471F4); // CShadows::RenderStoredShadows
	RET(g_libGTASA + 0x545C04); // CShadows::StoreStaticShadow
	RET(g_libGTASA + 0x36E77C); // CCamera::CamShake
