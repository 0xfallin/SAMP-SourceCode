...
void CPlayerPed::SetArmedWeapon(int iWeaponType)
{
	...
	((int(*)(uintptr_t, unsigned int))(g_libGTASA+0x434528+1))((uintptr_t)m_pPed, iWeaponType);
}
...
bool CPlayerPed::StartPassengerDriveByMode()
{
	if(m_pPed) 
	{
		...
		if(iWeapon != WEAPON_UZI && iWeapon != WEAPON_MP5 && iWeapon != WEAPON_TEC9 && iWeapon != WEAPON_SAWEDOFF)
			return false;
...
void CPlayerPed::ProcessParachuteSkydiving()
{
	...
	uint16_t wKeys = GetKeys(&lrAnalog, &udAnalog, &additionalKey);
	MATRIX4X4 mat;
