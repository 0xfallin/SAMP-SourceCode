...
CKeyBoard::CKeyBoard()
{
	...
	Log("Keyboard | Size: %f, %f. Pos: %f, %f", m_Size.x, m_Size.y, m_Pos.x, m_Pos.y);
	Log("Keyboard | font size: %f. Key's height: %f", m_fFontSize, m_fKeySizeY);
...
void CKeyBoard::Render()
{
	...
	ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x / 1.1735 - vecButSize.x, io.DisplaySize.y / 1.78 - vecButSize.y * 3.0));
	ImGui::Begin("keys", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_AlwaysAutoResize);

	if(ImGui::Button("<<", vecButSize)) 
	{
		if(m_iCurrentPosItem < m_iTotalLastItem)
		{
			InsertText(m_szLastItems[m_iCurrentPosItem]);
			m_iCurrentPosItem++;
		}
	}
	...
	if(ImGui::Button(">>", vecButSize)) 
	{
		...
		if(m_iCurrentPosItem > 0)
			InsertText(m_szLastItems[m_iCurrentPosItem - 1]);
	...
	else 
	{
		...
		ImGui::GetOverlayDrawList()->AddText(pGUI->GetFont(), m_fFontSize, ImVec2(m_Pos.x + m_Size.x * 0.02, m_Pos.y + m_Pos.y * 0.05), 0xFFFFFFFF, _utf8Input);
	}

	//dividing line
	ImGui::GetOverlayDrawList()->AddLine(ImVec2(m_Pos.x, m_Pos.y + m_fKeySizeY), ImVec2(m_Size.x, m_Pos.y + m_fKeySizeY), 0xFF3291F5);
	...
	for(int i=0; i<LAYOUT_ROWS_C; i++)
	{
		...
		//for( auto key : m_Rows[m_iLayout][i])
		{
			...
			if(key.id == m_iPushedKey && key.type != KEY_SPACE && !m_bHidenInput)
				ImGui::GetOverlayDrawList()->AddRectFilled(key.pos, ImVec2(key.pos.x + key.width, key.pos.y + fKeySizeY), 0xFF3291F5);
			...
			switch(key.type)
			{
				...
				case KEY_SHIFT:
					ImGui::GetOverlayDrawList()->AddTriangleFilled(ImVec2(key.pos.x + key.width * 0.37, key.pos.y + fKeySizeY * 0.50), ImVec2(key.pos.x + key.width * 0.50, key.pos.y + fKeySizeY * 0.25), ImVec2(key.pos.x + key.width * 0.63, key.pos.y + fKeySizeY * 0.50), m_iCase == LOWER_CASE ? 0xFF8A8886 : 0xFF3291F5);
					ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(key.pos.x + key.width * 0.44, key.pos.y + fKeySizeY * 0.5 - 1), ImVec2(key.pos.x + key.width * 0.56, key.pos.y + fKeySizeY * 0.68), m_iCase == LOWER_CASE ? 0xFF8A8886 : 0xFF3291F5);
				break;
				...
				case KEY_SWITCH:
					ImGui::GetOverlayDrawList()->AddText(pGUI->GetFont(), m_fFontSize, key.symPos, 0xFFFFFFFF, "123");
				break;

				case KEY_SPACE:
					ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(key.pos.x + key.width * 0.07, key.pos.y + fKeySizeY * 0.3), ImVec2(key.pos.x + key.width * 0.93, key.pos.y + fKeySizeY * 0.7), key.id == m_iPushedKey ? 0xFF3291F5 : 0xFF8A8886);
				break;

				case KEY_SEND:
					ImGui::GetOverlayDrawList()->AddTriangleFilled(ImVec2(key.pos.x + key.width * 0.3, key.pos.y + fKeySizeY * 0.25), ImVec2(key.pos.x + key.width * 0.3, key.pos.y + fKeySizeY * 0.75), ImVec2(key.pos.x + key.width * 0.7, key.pos.y + fKeySizeY * 0.50), 0xFF8A8886);
				break;
			}
		}
	}
...
bool CKeyBoard::OnTouchEvent(int type, bool multi, int x, int y)
{
	...
	if(x >= (io.DisplaySize.x / 1.1735) - (ImGui::GetFontSize() * 4) && y >= io.DisplaySize.y / 1.78 - (ImGui::GetFontSize() * 2.5) * 3 && y <=  io.DisplaySize.y / 1.78) 
		return true;
...
void CKeyBoard::Send()
{
	if(m_pHandler) 
	{
		if(m_sInput.size() && !pDialogWindow->m_bIsActive)
		{
			m_iCurrentPosItem = 0;
			m_iTotalLastItem++;
...
kbKey* CKeyBoard::GetKeyFromPos(int x, int y)
{
	...
	Log("Keyboard | Unknown key.");
	return nullptr;
}

//Maybe i will add for loops so dont waste the line -ZTzTopia
