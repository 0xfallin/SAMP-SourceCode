...
void PlayerSpectatePlayer(RPCParameters *rpcParams)
{
	...
	{
		case 2:
			byteMode = SPECTATING_MODE_FIXED;
			break;
		case 3:
			byteMode = SPECTATING_MODE_SIDE;
			break;
		default:
			byteMode = SPECTATING_MODE_PLAYER;
	}
	...
	if(pPlayerPool)
	{
		...
		if(pLocalPlayer)
		{
			pLocalPlayer->m_spectateData.m_byteSpectateMode = byteMode;
			pLocalPlayer->SpectatePlayer(playerId);
		}

		if(pPlayerPool->GetSlotState(pPlayerPool->GetLocalPlayerID()))
			pPlayerPool->GetAt(pPlayerPool->GetLocalPlayerID())->SetState(PLAYER_STATE_SPECTATING);
	}
}
...
void PlayerSpectateVehicle(RPCParameters *rpcParams)
{
	...
	switch (byteMode) 
	{
		...
		case 3:
			...
			break;
		default:
			byteMode = SPECTATING_MODE_VEHICLE;
	}
