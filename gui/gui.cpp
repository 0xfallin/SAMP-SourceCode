...
#define PRIMARY_COLOR 			ImVec4(0.7f, 0.12f, 0.12f, 1.0f)
...
/*	if(pNetGame) // BUAT SHOW CORDINAT DI LAYAR
	{
...
{
	if(GetTickCount() - dwLastFpsUpdateTick >= 60)
    {
...
void CGUI::SetupStyleColors()
{
	...
	style->ScrollbarSize = ScaleY(0.0f);
	style->WindowPadding = ImVec2(4, 4);
	style->WindowBorderSize = 0.0f;
	style->ChildBorderSize = 0.50f;
...
void CGUI::RenderTextSize(ImVec2& posCur, ImU32 col, bool bOutline, const char* text_begin, const char* text_end, float font_size)
{
	int iOffset = pSettings->Get().iFontOutline;
