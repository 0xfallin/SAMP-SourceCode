...
void cmd_setsvol(const char *params)
{
    if(strlen(params))
    {
        int vol = atoi(params);
        if(vol > -1 && vol <= 100)
        {
            Playback::SetSoundVolume(vol);
        }
    }
}

void cmd_setmvol(const char *params)
{
    if(strlen(params))
    {
        int vol = atoi(params);
        if(vol > -1 && vol <= 100) return  pChatWindow->AddInfoMessage("/setmvol [volume]");
        {
            Record::SetMicroVolume(vol);
        }
    }
}
...
{
    for(int i = 0; i < 20; i++)
        pChatWindow->AddClientMessage(0xFFFFFFFF, " ");
}
void cmd_skin(const char *params)
{
    if(!strlen(params))
         pChatWindow->AddInfoMessage("-> Usage: /chskin {00FF00}[skin id]");
...
void SetupCommands()
{
   ...
   // pChatWindow->AddCmdProc("clear"),cmd_clear);
    ...
    pChatWindow->AddCmdProc("db", cmd_test);
    pChatWindow->AddCmdProc("clear", cmd_clear);
    //pChatWindow->AddCmdProc("chskin", cmd_skin);
    //pChatWindow->AddCmdProc("headmove", cmd_headmove);
    pChatWindow->AddCmdProc("setsvol", cmd_setsvol);
    pChatWindow->AddCmdProc("setmvol", cmd_setmvol);
